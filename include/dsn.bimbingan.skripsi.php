...
$query = "SELECT 
		 pembimbing_skripsi.id_pem_skripsi,
		 pembimbing_skripsi.dosen1,
		 pembimbing_skripsi.nim,
		 mahasiswa.nim,
		 mahasiswa.nama,
		 mahasiswa.thn_msk,
		 konsentrasi.nm_konsentrasi
		 FROM pembimbing_skripsi,mahasiswa,konsentrasi 
		 where
		 (pembimbing_skripsi.dosen1='$id_dosen' or 
		 pembimbing_skripsi.dosen2='$id_dosen') and
		 pembimbing_skripsi.nim=mahasiswa.nim and 
...
?>
<br>
<h2>Halaman Daftar Bimbingan Skripsi Mahasiswa</h2>
<br>
<form action="index.php?target=dsn.bimbingan.skripsi" method="post" class="search">
<table width=100%>
<tr>
...
<tr style="border-bottom:1px solid #aaa;">
	...
	<td align=center><a href="index.php?target=dsn.look.data.skripsi&nim=<?php echo $data[3]; ?>">Look Data</a></td>
</tr>
<?php		
	$no++;
	}
...
if ($page > 1){
echo  "<a style='padding-top:3px;padding-bottom:3px;color:#333;font-family:calibri;padding-left:5px;padding-right:5px;background:#ffccd5;border:1px solid #ffccd5;text-decoration:none;' href='".$_SERVER['PHP_SELF']."?target=dsn.bimbingan.skripsi&batas=$batas&page=".($page-1)."&cari=$cari'><< Prev</a>";
}
...
//mengurutkan agar yang tampil i+3 dan i-3
         if ((($i >= $page - 3) && ($i <= $page + 3)) || ($i == 1) || ($i == $jumPage))
         {
            ...
            else echo " <a style='font-family:calibri;border:1px solid #ffccd5;text-decoration:none;color:#000;padding-top:3px;padding-bottom:3px;padding-left:8px;padding-right:8px;background:#ffccd5;' href='".$_SERVER['PHP_SELF']."?target=dsn.bimbingan.skripsi.kklp&batas=$batas&page=".$i."&cari=$cari'>".$i."</a> ";
            if($i==1 && $page >= 6) echo "<a href='' style='font-family:calibri;color:#000;padding-left:5px;padding-right:5px;'>...</a>";
         }
}
//menampilkan link Next >>
if ($page < $jumPage){
echo "<a style='font-family:calibri;color:#333;padding-top:3px;padding-bottom:3px;padding-left:5px;padding-right:5px;background:#ffccd5;border:1px solid #ffccd5;text-decoration:none;' href='".$_SERVER['PHP_SELF']."?target=dsn.bimbingan.skripsi.kklp&batas=$batas&page=".($page+1)."&cari=$cari'>Next >></a>";
}
?>
</div>
<br>