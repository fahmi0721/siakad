...
<form action="index.php?target=look.kklp" method="post" class="search">
<table width=100%>
<tr>
<td width=115 valign=middle>View&nbsp;  
<select name="view" class="select" onChange="MM_jumpMenu('parent',this,0)">
<option value="index.php?target=look.kklp&batas=5&cari=<?php echo $cari?>" <?php if ($batas==5) {echo "selected";} ?>>5</option>
<option value="index.php?target=look.kklp&batas=10&cari=<?php echo $cari?>" <?php if ($batas==10) {echo "selected";} ?>>10</option>
<option value="index.php?target=look.kklp&batas=20&cari=<?php echo $cari?>" <?php if ($batas==20) {echo "selected";} ?>>20</option>
<option value="index.php?target=look.kklp&batas=50&cari=<?php echo $cari?>" <?php if ($batas==50) {echo "selected";} ?>>50</option>
</select>
Entries
</td>
<td width=100><a href="index.php?target=input.kklp" class="tombol">Add Data</a></td>
<td align=right>
...
<th width=210>Instansi</th>
<th width=210>Alamat</th>
<th width=40>Aksi</th>
</tr>
<?php
...
$qry_kklp=mysql_query("select * from kklp where nim='$data[0]'");
$data_kklp=mysql_fetch_array($qry_kklp);
if($data_kklp[3]==""){
 $instansi="<font style='color:#f00;'>Belum Terdaftar</font>";
} else {
	$instansi="$data_kklp[3]";
}

if($data_kklp[4]==""){
	$alamat="<font style='color:#f00;'>Belum Terdaftar</font>";
} else {
	$alamat="$data_kklp[4]";
}
...
<td align=center valign=middle><a href="index.php?target=input.kklp&nim=<?php echo $data[0]; ?>&kd_mk=<?php echo $data[3]; ?>" class="table-actions-button ic-table-edit"></a> </td>
</tr>
<?php
$no++;
}
...
if ($page > 1){
echo  "<a style='padding-top:3px;padding-bottom:3px;color:#333;font-family:calibri;padding-left:5px;padding-right:5px;background:#d7ebf9;border:1px solid #d7ebf9;text-decoration:none;' href='".$_SERVER['PHP_SELF']."?target=look.kklp&batas=$batas&page=".($page-1)."&cari=$cari'><< Prev</a>";
}
...
//mengurutkan agar yang tampil i+3 dan i-3
         if ((($i >= $page - 3) && ($i <= $page + 3)) || ($i == 1) || ($i == $jumPage))
         {
            ...
            else echo " <a style='font-family:calibri;border:1px solid #d7ebf9;text-decoration:none;color:#000;padding-top:3px;padding-bottom:3px;padding-left:8px;padding-right:8px;background:#d7ebf9;' href='".$_SERVER['PHP_SELF']."?target=look.kklp&batas=$batas&page=".$i."&cari=$cari'>".$i."</a> ";
            if($i==1 && $page >= 6) echo "<a href='' style='font-family:calibri;color:#000;padding-left:5px;padding-right:5px;'>...</a>";
         }
}
//menampilkan link Next >>
if ($page < $jumPage){
echo "<a style='font-family:calibri;color:#333;padding-top:3px;padding-bottom:3px;padding-left:5px;padding-right:5px;background:#d7ebf9;border:1px solid #d7ebf9;text-decoration:none;' href='".$_SERVER['PHP_SELF']."?target=look.kklp&batas=$batas&page=".($page+1)."&cari=$cari'>Next >></a>";
}
?>
