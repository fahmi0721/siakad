...
$query = "SELECT 
		  ...
		  FROM mata_kuliah,rel_kon_ma,konsentrasi
		  where
		  mata_kuliah.kd_mk=rel_kon_ma.kd_mk and
		  konsentrasi.kd_konsentrasi=rel_kon_ma.kd_konsentrasi
		  LIMIT $offset, $limit";
...
$query = "SELECT 
		 ...
		 where
		 mata_kuliah.kd_mk=rel_kon_ma.kd_mk and
		 konsentrasi.kd_konsentrasi=rel_kon_ma.kd_konsentrasi and
		 (mata_kuliah.kd_mk like '%$cari%' or
		 mata_kuliah.nama_mk like '%$cari%' or 
		 mata_kuliah.sks like '%$cari%' or
		 mata_kuliah.semester like '%$cari%' or
		 mata_kuliah.ket like '%$cari%' or
...
?>
<form action="index.php?target=look.ketua.mk" method="post" class="search">
<table width=100%>
<tr>
<td width=115 valign=middle>View&nbsp;  
<select name="view" class="select" onChange="MM_jumpMenu('parent',this,0)">
<option value="index.php?target=look.ketua.mk&batas=5&cari=<?php echo $cari?>" <?php if ($batas==5) {echo "selected";} ?>>5</option>
<option value="index.php?target=look.ketua.mk&batas=10&cari=<?php echo $cari?>" <?php if ($batas==10) {echo "selected";} ?>>10</option>
<option value="index.php?target=look.ketua.mk&batas=20&cari=<?php echo $cari?>" <?php if ($batas==20) {echo "selected";} ?>>20</option>
<option value="index.php?target=look.ketua.mk&batas=50&cari=<?php echo $cari?>" <?php if ($batas==50) {echo "selected";} ?>>50</option>
</select>
Entries
</td>
<td align=right>
...
if ($page > 1){
echo  "<a style='padding-top:3px;padding-bottom:3px;color:#333;font-family:calibri;padding-left:5px;padding-right:5px;background:#d7ebf9;border:1px solid #d7ebf9;text-decoration:none;' href='".$_SERVER['PHP_SELF']."?target=look.ketua.mk&batas=$batas&page=".($page-1)."&cari=$cari'><< Prev</a>";
}
...
//mengurutkan agar yang tampil i+3 dan i-3
         if ((($i >= $page - 3) && ($i <= $page + 3)) || ($i == 1) || ($i == $jumPage))
         {
            ...
            else echo " <a style='font-family:calibri;border:1px solid #d7ebf9;text-decoration:none;color:#000;padding-top:3px;padding-bottom:3px;padding-left:8px;padding-right:8px;background:#d7ebf9;' href='".$_SERVER['PHP_SELF']."?target=look.ketua.mk&batas=$batas&page=".$i."&cari=$cari'>".$i."</a> ";
            if($i==1 && $page >= 6) echo "<a href='' style='font-family:calibri;color:#000;padding-left:5px;padding-right:5px;'>...</a>";
         }
}
//menampilkan link Next >>
if ($page < $jumPage){
echo "<a style='font-family:calibri;color:#333;padding-top:3px;padding-bottom:3px;padding-left:5px;padding-right:5px;background:#d7ebf9;border:1px solid #d7ebf9;text-decoration:none;' href='".$_SERVER['PHP_SELF']."?target=look.ketua.mk&batas=$batas&page=".($page+1)."&cari=$cari'>Next >></a>";
}
?>
